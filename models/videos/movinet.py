...
class HardSigmoid(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.inplace:
            return x.mul_(0.2).add_(0.5).clamp_(min=0.0, max=1.0)
...
class TemporalAverage(nn.Module):
    def __init__(self, causal=True):
        ...
        if self.causal:
            ...
            self.register_module('cgap', None)
...
class ConvBlock3D(CausalModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.tf_like:
            x = _same_padding(x, x.shape[-2], x.shape[-1],
    ...
    def __init__(self,
        ...
        for i, block in enumerate(cfg.blocks):
            for j, basic_block in enumerate(block):
                blocks_dict[f"b{i}_l{j}"] = BasicBottleneck(
                    basic_block,
                    causal=causal,
                    conv_type=conv_type,
                    tf_like=tf_like,
        ...
        # pool
        self.avg = TemporalAverage(self.causal)
...
def movinet_a0(**kwargs):
    """Constructor for the MoViNet A0 network
    """
    return MoViNet(cfg=get_config('movinet_a0'), **kwargs)


def movinet_a1(**kwargs):
    """Constructor for the MoViNet A0 network
    """
    return MoViNet(cfg=get_config('movinet_a1'), **kwargs)


def movinet_a2(**kwargs):
    """Constructor for the MoViNet A0 network
    """
    return MoViNet(cfg=get_config('movinet_a2'), **kwargs)


def movinet_a3(**kwargs):
    """Constructor for the MoViNet A0 network
    """
    return MoViNet(cfg=get_config('movinet_a3'), **kwargs)


def movinet_a4(**kwargs):
    """Constructor for the MoViNet A0 network
    """
    return MoViNet(cfg=get_config('movinet_a4'), **kwargs)


def movinet_a5(**kwargs):
    """Constructor for the MoViNet A0 network
    """
    return MoViNet(cfg=get_config('movinet_a5'), **kwargs)
